...
- Key environmental variables used in the final model are:
  ...
  - chlorophyll-a
- Some exploratory work also uses Queensland land use data and clean water data to visually examine possible spatial relationships between coastal activity and reef conditions. :contentReference[oaicite:2]{index=2}
...
The EDA first maps sugar land use in Queensland and compares it with the spatial distribution of COTS observations. It then overlays the two to visually inspect whether COTS activity appears more concentrated near coastal farming regions. The EDA also filters COTS observations near the coastline and creates year-based animated maps to show how outbreaks vary across time and space. :contentReference[oaicite:3]{index=3}

### 2. Data Preparation

The modelling pipeline starts by cleaning the COTS dataset and renaming core variables such as longitude, latitude, survey date, total COTS, mean COTS per tow, and coral cover. It keeps observations after 2009. Then it downloads annual eReefs data and extracts DIN, DIP, DIC, and chlorophyll values across latitude, longitude, time, and depth. To avoid duplicate depth layers, the workflow keeps the shallowest available depth for each location-date combination. :contentReference[oaicite:4]{index=4}
...
These transformed predictors are used together with DIC and live coral cover in the final classification models. :contentReference[oaicite:5]{index=5}
